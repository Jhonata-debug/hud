...
local setaEsquerda = false
local setaDireita = false
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSYSTEM
-----------------------------------------------------------------------------------------------------------------------------------------
	...
	while true do
		...
		if LocalPlayer["state"]["Active"] and Display then
			...
			if IsPedInAnyVehicle(Ped) then
				...
				if (lights == 1 and highlights == 0) or (lights == 1 and highlights == 1) or (lights == 0 and highlights == 1) then 
					farol = true
				end
					...
					if Entity(Vehicle)["state"]["Drift"] then
						...
						if (Class >= 0 and Class <= 7) or Class == 9 then
							if IsControlPressed(1, 21) then
								if VSpeed <= 100.0 and not GetDriftTyresEnabled(Vehicle) then
				...
				if ActualVehicle ~= Vehicle then
					SendNUIMessage({ Action = "Vehicle", Vehicle = true, Number = Armouring })
				...
				if Speed ~= VSpeed then
					SendNUIMessage({ Action = "Speed", Number = VSpeed, maxSpeed = maxSpeed, Rpm = Rpm, Gear = Gear })
					Speed = VSpeed
				end
				...
				SendNUIMessage({ Action = "Gear", Gear = Gear })
				SendNUIMessage({ Action = "Engine", Number = parseInt(engine/10)})
				SendNUIMessage({ Action = "Headlight", Status = farol})
				...
				if ActualVehicle or true then
					ActualVehicle = nil
					SendNUIMessage({ Action = "Vehicle", Vehicle = false, Number = Armouring })
					...
					SendNUIMessage({ Action = "Headlight", Status = false })
...
-- KEYMAPPING SETAS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterKeyMapping("+setaEsquerda","Seta esquerda.","keyboard","LEFT")
RegisterKeyMapping("+setaDireita","Seta direita.","keyboard","RIGHT")
function setaEsquerda()
	local Ped = PlayerPedId()
	if IsPedInAnyVehicle(Ped) then
		if setaEsquerda then
			SendNUIMessage({ Action = "SetaEsquerda", Status = false })
			setaEsquerda = false
		else
			SendNUIMessage({ Action = "SetaEsquerda", Status = true })
			setaEsquerda = true
		end
	end
end
...
function setaDireita()
	...
	if IsPedInAnyVehicle(Ped) then
		if setaDireita then
			SendNUIMessage({ Action = "SetaDireita", Status = false })
			setaDireita = false
		else
			SendNUIMessage({ Action = "SetaDireita", Status = true })
			setaDireita = true
		end
	end
end
RegisterCommand("+setaEsquerda",setaEsquerda)
RegisterCommand("+setaDireita",setaDireita)
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADMANEUVERS
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
